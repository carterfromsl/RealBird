...
class SellingPlansWidget {
  ...
  */
  ...
  get visibleSellingPlanForm() {
    return this.shopifySection.querySelector(`div[data-variant-id^="${this.variantIdInput.value}"]`);
  }
  ...
  */
  handleSellingPlanFormVisibility() {
    const sellingPlanFormForSelectedVariant = this.shopifySection.querySelector(`div[data-variant-id="${this.variantIdInput.value}"]`);
    const sellingPlanFormsForUnselectedVariants = this.shopifySection.querySelectorAll(
...
document.querySelectorAll('.selling_plan_app_container').forEach((sellingPlansWidgetContainer) => {
